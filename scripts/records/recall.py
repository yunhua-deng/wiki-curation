...
from scripts.wiki_index.fts_text import to_match_expr
...
def _fts_query_or(text: str) -> str:
    ...
    先按非词字符切分（URL 也会拆成 host/路径关键词），每个 token 经 to_match_expr
    转成表达式（CJK 逐字成短语，从而子串可命中；长度 >= BIGRAM_MIN_CJK 的无空格
    中文长串再 OR 上相邻二字组，避免整串短语召回为空），OR 语义保证部分命中也能召回。
    """
    ...
    return " OR ".join(to_match_expr(t) for t in tokens[:12])
