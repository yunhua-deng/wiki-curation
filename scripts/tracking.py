...
## 近期线索
近 45 天内关联记录中的新动态（2-5 句，可概括最新几条记录的核心信息）+ 本次 refresh 新材料中的新信息；两者都无才写「暂无新线索」。

## 已关联记录
按日期倒序列表：- [id]（date）**标题**——一句话说明

## 来源
**已登记来源**（系统会定期抓取；只列系统提供的已知来源，全部用 `[说明](url)` markdown 链接）：
- [个人主页](https://...)
（无登记来源则写「暂未登记」）

**建议来源**（你在材料/关联记录中发现、值得系统登记的新 URL；全部用 markdown 链接，并一句话说明为什么值得跟踪；人工审阅后登记，登记后进入下轮抓取）：
- [Google Scholar](https://...)——建议理由
```

## 硬性规则

1. 内容可溯源到关联记录或已抓材料；**禁止编造 URL**。
2. **已关联记录与动态时间线中的每一条都必须包含：id、日期、标题、一句话说明**——不许只列 id。
3. 来源章节分「**已登记来源**」与「**建议来源**」两小节；所有 URL 必须写成 `[说明](url)` markdown 链接，禁止裸 URL；**禁止出现"建议登记"字样**。
4. 客观陈述，不评价。
5. **禁止执行任何 git 操作**；不要写 digest.new.md 以外的文件；不要运行 publish / cli 命令。
6. 完成后只返回一句话摘要。
"""
